...
class GroqProvider implements LLMProviderService {
  final String apiKey;
  final String modelName;

  GroqProvider({required this.apiKey, required this.modelName});
  ...
  Future<String> generateResponse({
    ...
    if (apiKey.isEmpty) {
      throw Exception('No Groq API key provided');
    }
    ...
    final response = await http.post(
      ...
      body: jsonEncode({
        'model': modelName.isEmpty ? 'llama-3.1-8b-instant' : modelName,
        'messages': messages,
      }),
    );
    ...
    if (text == null || text.isEmpty) {
      throw Exception('Groq API returned an empty text message.');
    }
